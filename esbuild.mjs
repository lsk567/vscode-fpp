...
const nodeContext = await esbuild.context({
    entryPoints: [
        ...
        'packages/extension/parser/worker.ts',
    ],
    outdir: 'pack/extension',
    entryNames: '[name]', // Make sure extension.js and worker.js are placed under `pack/`
    bundle: true,
    target: 'ES2021',
    format: 'cjs',
    loader: { '.ts': 'ts' },
...
const browserContext = await esbuild.context({
    entryPoints: ['packages/webview/src/main.ts'],
    outdir: 'pack/diagram',
    bundle: true,
    target: 'ES2022',
