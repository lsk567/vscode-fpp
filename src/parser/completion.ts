...
export class CandidatesCollection {
    ...
    public ruleOffsets: Map<number, number> = new Map();
    public ruleOffsetsAtCaret: Set<number> = new Set();
}
...
class FollowSetWithPath {
    public intervals: IntervalSet = new IntervalSet();
...
class FollowSetsHolder {
    ...
    public combined: IntervalSet = new IntervalSet();
    public isExhaustive: boolean = false;
}
...
export class CodeCompletionCore {
    ...
    public collectCandidates(caretTokenIndex: number, context?: ParserRuleContext): CandidatesCollection {
        ...
        this.candidates.ruleOffsets.clear();
        this.candidates.ruleOffsetsAtCaret.clear();
        ...
        while (true) {
            ...
            if (token.startIndex >= caretTokenIndex + 1) {
                break;
            }
            ...
            if (token.channel === Token.DEFAULT_CHANNEL) {
                ...
                if (token.stopIndex + 1 >= caretTokenIndex || token.type === Token.EOF) {
                    break;
                }
            } else if (token.stopIndex + 1 >= caretTokenIndex) {
                this.tokens.push(token);
                break;
            }
    ...
    private addStateOffset(state: ATNState, atCaret: boolean) {
        if (atCaret || !this.candidates.ruleOffsetsAtCaret.has(state.ruleIndex)) {
            const ruleStartState = this.atn.ruleToStartState[state.ruleIndex].transition(0).target.stateNumber;
            this.candidates.ruleOffsets.set(
                state.ruleIndex,
                state.stateNumber - ruleStartState
            );

            if (atCaret) {
                this.candidates.ruleOffsetsAtCaret.add(state.ruleIndex);
            }
        }
    }
    ...
    private processRule(startState: RuleStartState, tokenListIndex: number, callStack: RuleWithStartTokenList,
        ...
        let currentEntry: IPipelineEntry;
        ...
        while (statePipeline.length > 0) {
            ...
            this.addStateOffset(currentEntry.state, atCaret);
